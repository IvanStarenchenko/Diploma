...
import { getProductById } from '../../redux/catalogReducer'
import { compose } from 'redux';
import { connect } from "react-redux"
const Catalog = (props) => {
    const [addedItems, setAddedItems] = useState(new Set());

    // Загружаем состояние из `props.favoriteContent` при монтировании компонента
    useEffect(() => {
        const favoriteItemIds = new Set(props.favoriteContent.map((item) => item.id));
        setAddedItems(favoriteItemIds);
    ...
    return (
        <div className="catalog-products__cards" id="recommended">
            {props.product.map(item => (
                <div key={item.productId} className="catalog-products__card card">
                    {!addedItems.has(item.productId)
                        ...
                        : <button className="card__fav" onClick={() => deleteItem(item.productId)}>
                    ...
                    <NavLink to={`/Description/${item.productId}`} onClick={() => props.getProductById(item.productId)}>
                        <img className="card__image" src={item.itemImageMain} alt='' />
                    </NavLink>

                    <div className="card__content">
                        <div className="card__text">
                            <div className="card__name">{item.productName}</div>
                            <div className="card__brand">{item.manufacturer}</div>
                        </div>
...
export default connect(mapStateToProps, { addItemToFav, deleteItemFromFav , getProductById })(Catalog);
